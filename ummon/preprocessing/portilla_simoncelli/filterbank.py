...
def visualize(coeff, normalize = True):
    """
    To visualize the filter coefficients, which are saved in a list of lists of numpy arrays,
    this function brings all the sub-bands into one image.
    Args:
        coeff:      filter pyramid coefficients
        normalize:  normalize to get image values scaled in-between 0-255
    ...
    out = np.zeros((M * 2 - coeff[-1][0].shape[0], Norients * N))
    ...
    m,n = coeff[-1][0].shape
    out[currentx:currentx+m, currenty:currenty+n] = 255 * np.divide(coeff[-1][0],coeff[-1][0].max())
...
class Steerable:
    ...
    def buildSCFpyr(self, im):
        assert im.ndim == 2, 'Input image must be grayscale'
    ...
    def reconSCFpyr(self, coeff):
        ...
        res = np.fft.ifft2(np.fft.ifftshift(outdft)).real.astype(int)
