...
from .config import FILTERPYRAMID, PLOT_CORR, SAVE_PARAMS, COMPARE_COEFF, NA, \
        FULLY_COVERED
from . import filterbank_simoncelli
from .auxiliary import expand, norm, random_noise
from .filterbank import Steerable
from .moments import skew2, kurt2
from .params import Params
from .test_pyramid import compare_coefficients
...
class Analysis:
    ...
    Attributes:
        ...
        xCorrReal:      cross-scale correlation of real sub-bands
                        (between orientations of two adjacent scales for each scale)
    """
    def __init__(self, img, nsc=4, pixelstats=0, Na=0):
        """
        ...
        Args:
            ...
            pixelstats:     overwrite pixelstats with given pixelstats of another image (for comparing studies)
            Na:             spatial neighborhood of autocorrelations is Na x Na coefficients (must be odd)
    ...
    def noiseImg(self):
        """
        Add noise to image to avoid instability created by symmetric conditions at the synthesis process.
        """
        self.img = self.img + (self.pixelStats[5]-self.pixelStats[4])/1000*random_noise(0, 1,self.img.shape)
        return self.img
    ...
    def buildSteerablePyramid(self):
        ...
        if COMPARE_COEFF:
            compare_coefficients(self.coeff)
    ...
    def getFeatures(self):
        ...
        # save analysis params to test in matlab
        if SAVE_PARAMS:
            savemat("../matlab/textureSynth/python_params.mat", params_dic)
