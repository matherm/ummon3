    ...
    def evaluate(model, loss_function, dataset, logger=Logger(), after_eval_hook=None, batch_size=-1):
        ...
        for i, data in enumerate(dataloader, 0):
                ...
                if after_eval_hook is not None:
                    after_eval_hook(ctx, uu.tensor_tuple_to_data(output), targets.data, loss.data)
...
class SiameseAnalyzer(SupervisedAnalyzer):
    ...
    def evaluate(model, loss_function, dataset, logger=Logger(), after_eval_hook=None, batch_size=-1):
        ...
        for i, data in enumerate(dataloader, 0):
                ...
                # Run hook
                if after_eval_hook is not None:
                    after_eval_hook(ctx, uu.tensor_tuple_to_data(output), uu.tensor_tuple_to_data(targets), loss.data)
